...
public interface ProtocalRepository extends BaseRepository<ProtocalDO, Long> {
    @Query("select d from Protocal d where concat(coalesce(d.name, ''), coalesce(d.startFlag, ''), coalesce(d.endFlag, ''), coalesce(d.split, ''), coalesce(d.offset, 0), coalesce(d.offsetType, ''), coalesce(d.count, 0), coalesce(d.eqpName, '')) like :keyword")

...
<script>
    $(function() {
        createBootstrapTable('#table', '/lb/weight.do', ['ip', 'port', 'weight', 'ownerName', 'groupName'], ['IP', 'PORT', '权重', '设备类型名称', '分组名称'], true, '#toolbar', 'GET');
        ...
        $("#form").submit(function() {
            ...
            $.post("/lb/weight.do", params, function(result) {
                if (result.success) {
                    toastr.success('保存成功.');
                    $('#table').bootstrapTable('refresh', {silent: true});
                    $('#addModal').modal('hide');
